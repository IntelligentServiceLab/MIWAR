...
import random
from data_process import get_bert_emb,get_lightgcl_data
...
def main():
    ...
    model = LightGCL(adj_norm.shape[0], adj_norm.shape[1], d, u_mul_s, v_mul_s, svd_u.T, svd_v.T, train_csr, adj_norm,l, temp, lambda_1, lambda_2, dropout, batch_user, device,input_dim=768,hidden_dim=512,out_dim=d)
    model.to(device)
    ...
    for epoch in tqdm(range(1, config.epoch + 1),total=config.epoch):
        ...
        optimizer.zero_grad()
        loss, loss_r, loss_s,mashup_desc_output,pos_api_desc_output,neg_api_desc_output,mashup_stru_output,api_stru_output = model(uids,  pos, neg,pos_api_emb=pos_api_emb,neg_api_emb=neg_api_emb,mashup_emb=mashup_emb)
